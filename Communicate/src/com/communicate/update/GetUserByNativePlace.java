...
public class GetUserByNativePlace {
private List<PeopleItem> list ;
	
	public List<PeopleItem> getUserByNativePlace(String nativePlace){
		...
		try{
			...
			String query=("SELECT * FROM `us_er` WHERE `native_place` like '%" + nativePlace + "%'");// 创造SQL语句 
			...
			while(rs.next())
			{
				...
				String nativePlace2 =rs.getString("native_place");
		        ...
		        peopleItem.setNativePlace(nativePlace2);
